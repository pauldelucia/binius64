...
pub fn linearized_transform_scalar(x: &mut Ghash, table: &'static [[Ghash; 256]; BYTES_PER_GHASH]) {
	// Iterate the bytes of the element LSB-to-MSB (the same order the former
	// `<u128 as Divisible<u8>>::ref_iter` produced before the underlier moved to `M128`).
	*x = u128::from(*x)
		.to_le_bytes()
		.iter()
		.zip(table)
...
mod tests {
	...
	fn test_mds() {
		...
		// (2 is nonzero, so `invert_or_zero` returns its true inverse.)
		let x_inv = Ghash::new(2).invert_or_zero();
		let x = Ghash::new(2);
